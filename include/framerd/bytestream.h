...
#define FD_STREAM_LOCKED         (FD_STREAM_FLAGS * 1)
#define FD_STREAM_READ_ONLY      (FD_STREAM_FLAGS * 2)
#define FD_STREAM_WRITE_ONLY     (FD_STREAM_FLAGS * 4)
#define FD_STREAM_CANSEEK        (FD_STREAM_FLAGS * 8)
#define FD_STREAM_NEEDS_LOCK     (FD_STREAM_FLAGS * 16)
#define FD_STREAM_FILE_LOCKED    (FD_STREAM_FLAGS * 32)
#define FD_STREAM_SOCKET         (FD_STREAM_FLAGS * 64)
#define FD_STREAM_DOSYNC         (FD_STREAM_FLAGS * 128)
...
#define fd_getpos(s) \
  ((((s)->buf_flags)&FD_STREAM_CANSEEK) ?				\
   ((((s)->stream_filepos)>=0) ?						\
    ((FD_BYTESTREAM_ISREADING(s)) ?					\
     (((s)->stream_filepos)-(((s)->buflim)-((s)->bufpoint))) :		\
     (((s)->stream_filepos)+(((s)->bufpoint)-((s)->bufbase)))) :		\
    (_fd_getpos(s)))							\
   : (-1))
...
#define FD_BYTESTREAM_ISREADING(s) (!(U8_BITP((s->buf_flags),(FD_ISWRITING))))
#define FD_BYTESTREAM_ISWRITING(s) (U8_BITP((s->buf_flags),(FD_ISWRITING)))
