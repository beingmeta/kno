...
typedef struct KNO_EVALSERVER {
  ...
  union {struct U8_CONNPOOL *connpool;} connection;}
  KNO_EVALSERVER;
typedef struct KNO_EVALSERVER *kno_evalserver;

KNO_EXPORT lispval kno_init_evalserver(struct KNO_EVALSERVER *evalserver,u8_string server,lispval opts);
KNO_EXPORT struct KNO_EVALSERVER *kno_open_evalserver(u8_string server,lispval opts);
