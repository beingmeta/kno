...
typedef struct FDSERVLET {
  const char *sockname;
  time_t spawning, spawned;
  int servlet_index;
  struct sockaddr *addr;
  pthread_mutex_t *lock; /* Lock to get/add sockets */
...
static pthread_mutex_t *servlets_lock;
...
static unsigned char *ngx_pstrdup(ngx_pool_t *p,unsigned char *s)
{
  int len=strlen(s);
  unsigned char *copy=ngx_palloc(p,len+1);
  memcpy(copy,s,len+1);
  return copy;
}
...
static fdservlet servlet_set_keep_socks(fdservlet s,int keep_socks)
{
  if (s->keep_socks>=keep_socks) return s;
	...
	ngx_pcalloc(fdserv_pool,sizeof(struct FDSOCKET)*keep_socks);
...
static fdservlet add_servlet(struct ngx_http_request_t *r,const char *sockname,
			     int keep_socks,int max_socks)
{
  int i=0; int lim=n_servlets;
  pthread_mutex_lock(servlets_lock);
  while (i<lim) {
	...
	servlet_set_keep_socks(&(servlets[i]),keep_socks);
      ...
      pthread_mutex_unlock(servlets_lock);
      return &(servlets[i]);}
    else i++;}
  if (i>=max_servlets) {
      ...
      return NULL;}}
  {
    ...
    servlet->sockname=ngx_pstrdup(fdserv_pool,sockname);
    ...
    thread_mutex_create(&(servlet->lock));
...
static int get_servlet_wait(ngx_http_request_t *r);

/* Opens a servlet socket, either a cached one (if given != NULL) or a
   new one (malloc) */
static fdsocket servlet_open(fdservlet s,struct FDSOCKET *given,
			     ngx_http_request_t *r)
{
  struct FDSOCKET *result; ngx_pool_t *pool; int one=1, dospawn=-1, wait=-1;
  if (given) pool=fdserv_pool; else pool=r->pool;
  if (given) result=given;
  else {
    result=ngx_pcalloc(pool,sizeof(struct FDSOCKET));
...
static int get_servlet_wait(ngx_http_request_t *r)
{
  ...
  struct FDSERV_LOC_CONFIG *dconfig=
    ap_get_module_config(r->per_loc_config,&fdserv_module);
...
static fdsocket servlet_connect(fdservlet s,ngx_http_request_t *r)
{
  char infobuf[512];
  pthread_mutex_lock(s->lock); {
...
static fdservlet request_servlet(ngx_http_request_t *r)
{
  const char *sockname=get_sockname(r);
  struct FDSERV_LOC_CONFIG *config=
  ...
  if ((config)&&(config->keep_socks>keep_socks))
    keep_socks=config->keep_socks;
  if ((config)&&(config->max_socks>max_socks))
    max_socks=config->max_socks;
...
static BUFF *ap_bcreate(ngx_pool_t *p,int ignore_flag)
{
  struct BUFF *b=ngx_pcalloc(p,sizeof(struct BUFF));
  if (!(b)) return b;
  b->p=p; b->ptr=b->buf=ngx_pcalloc(p,4096); b->lim=b->buf+4096;
  return b;
}
...
static int sock_write(ngx_http_request_t *r,
...
static int copy_servlet_output(fdsocket sockval,ngx_http_request_t *r)
{
  char infobuf[256];
...
static int checkabort(ngx_http_request_t *r,fdservlet servlet,fdsocket sock,
		      int started)
{
  char infobuf[512];
  if (r->connection->aborted) {
...
static int executable_filep(ngx_pool_t *p,const char *filename)
{
  apr_finfo_t finfo;
...
static char *makepath(ngx_pool_t *p,const char *root,const char *name)
{
...
static int get_file_info(ngx_pool_t *p,const char *filename,fileinfo *info)
{
  return apr_stat(info,filename,FINFO_FLAGS,p);
}
static int stat_can_writep(ngx_pool_t *p,server_rec *s,apr_finfo_t *finfo)
{
  apr_uid_t uid; apr_gid_t gid;
  apr_uid_current(&uid,&gid,p);
  ap_log_error
...
static int file_existsp(ngx_pool_t *p,const char *filename)
{
  fileinfo finfo;
...
static int file_writablep(ngx_pool_t *p,server_rec *s,const char *filename)
{
...
static int file_socket_existsp(ngx_pool_t *p,server_rec *s,const char *filename)
{
  fileinfo finfo; int retval;
  ap_log_error(APLOG_MARK,LOGDEBUG,OK,s,
...
static const char *get_sockname(ngx_http_request_t *r)
{
...
static void *create_server_config(ngx_conf_t *cf)
{
  ...
  struct FDSERV_SERVER_CONFIG *config=
    ngx_pcalloc(p,sizeof(struct FDSERV_SERVER_CONFIG));
...
static void *merge_server_config(ngx_conf_t *cf,void *base,void *new)
{
  ngx_pool_t *p=cf->pool;
  struct FDSERV_SERVER_CONFIG *config=
    ngx_pcalloc(p,sizeof(struct FDSERV_SERVER_CONFIG));
  ...
  if (child->config_args) {
    ...
    fresh=ngx_pcalloc(p,(n_config+1)*sizeof(char *));
  ...
  if (child->req_params) {
    ...
    fresh=ngx_pcalloc(p,(n_slots+1)*sizeof(char *));
...
static void *create_loc_config(ngx_conf_t *cf,char *dir)
{
  ngx_pool_t *p=cf->pool;
  struct FDSERV_LOC_CONFIG *config=
    ngx_pcalloc(p,sizeof(struct FDSERV_LOC_CONFIG));
  config->server_executable=NULL;
  config->config_args=NULL;
  config->req_params=NULL;
  config->servlet_env=NULL;
...
static void *merge_loc_config(ngx_conf_t *cf,void *base,void *new)
{
  ngx_pool_t *p=cf->pool;
  struct FDSERV_LOC_CONFIG *config
    =ngx_pcalloc(p,sizeof(struct FDSERV_LOC_CONFIG));
  struct FDSERV_LOC_CONFIG *parent=base;
  struct FDSERV_LOC_CONFIG *child=new;

  memset(config,0,sizeof(struct FDSERV_LOC_CONFIG));
...
{
  struct FDSERV_LOC_CONFIG *dconfig=mconfig;
  struct FDSERV_SERVER_CONFIG *sconfig=
...
static const char *log_sync(cmd_parms *parms,void *mconfig,const char *arg)
{
  ...
  LOG_CONFIG(parms,arg);
  if (parms->path) {
    struct FDSERV_LOC_CONFIG *dconfig=(struct FDSERV_LOC_CONFIG *)mconfig;
...
static char **extend_config(ngx_pool_t *p,char **config_args,const char *var,const char *val);
...
static char **extend_config(ngx_pool_t *p,char **config_args,const char *var,const char *val)
{
  ...
  if (config_args==NULL) {
    char **vec=ngx_pcalloc(p,2*sizeof(char *));
...
static char **extend_params(ngx_pool_t *p,char **req_params,
...
static char **extend_params(ngx_pool_t *p,char **req_params,const
			    char *var,const char *val)
{
  if (req_params==NULL) {
    char **vec=ngx_pcalloc(p,3*sizeof(char *));
...
static int spawn_fdservlet (fdservlet s,ngx_http_request_t *r,ngx_pool_t *);
...
static int check_directory(ngx_pool_t *p,const char *filename)
{
...
static const char *get_log_file(ngx_http_request_t *r,const char *sockname) /* 2.0 */
{
  struct FDSERV_SERVER_CONFIG *sconfig=
    ap_get_module_config(r->server->module_config,&fdserv_module);
  struct FDSERV_LOC_CONFIG *dconfig=
...
static int spawn_wait(fdservlet s,ngx_http_request_t *r) ;

static int spawn_fdservlet(fdservlet s,ngx_http_request_t *r,ngx_pool_t *p) 
{
  const char *sockname=s->sockname;
  ...
  const char **loc_configs=(dconfig->config_args);
  ...
  *write_argv++=(char *)sockname;
  ...
  if (server_configs) {
	...
	*write_argv++=(char *)(*scan_config);}
      ...
      n_configs++;}}
  if (loc_configs) {
    const char **scan_config=loc_configs;
    while (*scan_config) {
      if (n_configs>MAX_CONFIGS) {
	ap_log_error(APLOG_MARK,APLOG_CRIT,OK,server,
  ...
  *write_argv++=NULL; n_configs++;
  ...
  if (dir_env) {
    const char **scan_env=loc_configs;
    while (*scan_env) {
      if (n_configs>MAX_CONFIGS) {
	ap_log_error(APLOG_MARK,APLOG_CRIT,OK,server,
...
static int spawn_wait(fdservlet s,ngx_http_request_t *r) 
{
  ngx_pool_t *p=((r==NULL)?(fdserv_pool):(r->pool));
  const char *sockname=s->sockname;
  struct FDSERV_SERVER_CONFIG *sconfig=
    ap_get_module_config(r->server->module_config,&fdserv_module);
  struct FDSERV_LOC_CONFIG *dconfig=
...
static int fdserv_handler(ngx_http_request_t *r)
{
  time_t started=time(), connected, requested, computed, responded;
  BUFF *reqdata;
  ...
  if (r->method_number == M_POST) {
    ...
    if (rv!=OK) {}
      ...
      char *data=ngx_pcalloc(r->pool,16384);
	...
	if (size+bytes_read > limit) {
	  char *newbuf=ngx_pcalloc(r->pool,limit*2);
...
static int fdserv_post_config(ngx_pool_t *p,
			      ngx_pool_t *plog,
			      ngx_pool_t *ptemp,
			      server_rec *s)
{
...
static void fdserv_init(ngx_pool_t *p,server_rec *s)
{
  ap_log_perror(APLOG_MARK,APLOG_INFO,OK,p,
  ...
  servlets=ngx_pcalloc(fdserv_pool,sizeof(struct FDSERVLET)*(FDSERV_INIT_SERVLETS));
...
static void register_hooks(ngx_pool_t *p)
{
...
module AP_MODULE_DECLARE_DATA fdserv_module =
{
    ...
    create_loc_config,				/* dir config creater */
    merge_loc_config,				/* dir merger --- default is to override */
