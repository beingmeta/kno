  ...
  if ( (STRINGP(opts)) && (encoding=u8_get_encoding(CSTRING(opts))) )
	...
	*open_flags |= O_WRONLY;
	...
	if (strchr(string,'c')) *open_flags = *open_flags | O_CREAT;}
      if (lock_flags) {
	if (strchr(string,'*')) *lock_flags = *lock_flags | 1;}}
    if (strchr(string,';')) {
    ...
    if (KNO_STRINGP(enc))
      encoding = u8_get_encoding(KNO_CSTRING(enc));
    else if (KNO_SYMBOLP(enc))
      encoding = u8_get_encoding(KNO_SYMBOL_NAME(enc));
    else NO_ELSE;
...
DEFC_PRIM("open-output-file",open_output_file,
...
DEFC_PRIM("extend-output-file",extend_output_file,
	  KNO_MAX_ARGS(3)|KNO_MIN_ARGS(1),
...
DEFC_PRIM("open-input-file",open_input_file,
...
DEFC_PRIM("write-file",writefile_prim,
...
static lispval simple_fileout_evalfn(lispval expr,kno_lexenv env,kno_stack _stack)
{
  ...
  lispval filename_val = kno_eval_arg(filename_arg,env,_stack);
...
static lispval simple_system_evalfn(lispval expr,kno_lexenv env,kno_stack _stack)
{
  ...
  {lispval string_exprs = kno_get_body(expr,1);
    KNO_DOLIST(string_expr,string_exprs) {
      lispval value = kno_eval(string_expr,env,_stack);
...
DEFC_PRIM("open-socket",open_socket_prim,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
...
DEFC_PRIM("remove-file!",remove_file_prim,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
...
DEFC_PRIM("remove-tree!",remove_tree_prim,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
...
DEFC_PRIM("move-file!",move_file_prim,
	  KNO_MAX_ARGS(3)|KNO_MIN_ARGS(2),
...
DEFC_PRIM("link-file!",link_file_prim,
	  KNO_MAX_ARGS(3)|KNO_MIN_ARGS(2),
...
DEFC_PRIM("filestring",filestring_prim,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
...
DEFC_PRIM("filedata",filedata_prim,
...
DEFC_PRIM("filecontent",filecontent_prim,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
...
DEFC_PRIM("set-file-access!",set_file_access_prim,
...
static lispval snapshot_evalfn(lispval expr,kno_lexenv env,kno_stack _stack)
{
  ...
  lispval arg1 = kno_eval_arg(kno_get_arg(expr,1),env,_stack), arg2 = kno_eval(kno_get_arg(expr,2),env,_stack);
  if (VOIDP(arg1)) {
...
DEFC_PRIM("zpathstore",zpathstore_prim,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"fname",kno_string_type,KNO_VOID},
	  {"opts",kno_any_type,KNO_VOID})
