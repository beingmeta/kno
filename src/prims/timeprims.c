...
static lispval millisecond_symbol, milliseconds_symbol;
static lispval microsecond_symbol, microseconds_symbol;
static lispval nanosecond_symbol, nanoseconds_symbol;
static lispval picosecond_symbol, picoseconds_symbol;
static lispval femtosecond_symbol, femtoseconds_symbol;
...
DEFC_PRIM("timestamp?",timestampp,
...
DEFC_PRIM("timestamp",timestamp_prim,
...
static lispval timestamp_prim(lispval arg)
{
  ...
  if (VOIDP(arg)) {
    ...
    return LISP_CONS(tm);}
  else if (KNO_KEYMAPP(arg)) {
    lispval base = kno_make_timestamp(NULL);
    u8_xtime xt = &(((struct KNO_TIMESTAMP *)(base))->u8xtimeval);
    lispval keys = kno_getkeys(arg);
    DO_CHOICES(key,keys) {
      lispval val = kno_get(arg,key,KNO_VOID);
      int rv = xtime_set(xt,key,val);
      kno_decref(val);
      if (rv<0) {
	kno_decref(base);
	kno_decref(keys);
...
DEFC_PRIM("gmtimestamp",gmtimestamp_prim,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(0),
	  "**undocumented**",
	  {"arg",kno_any_type,KNO_VOID})
...
DEFC_PRIM("timestamp+",timestamp_plus,
...
DEFC_PRIM("timestamp-",timestamp_minus,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"arg1",kno_any_type,KNO_VOID},
	  {"arg2",kno_any_type,KNO_VOID})
...
DEFC_PRIM("difftime",timestamp_diff,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"timestamp1",kno_any_type,KNO_VOID},
	  {"timestamp2",kno_any_type,KNO_VOID})
...
DEFC_PRIM("time-until",time_until,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"arg",kno_any_type,KNO_VOID})
...
DEFC_PRIM("time-since",time_since,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"arg",kno_any_type,KNO_VOID})
...
DEFC_PRIM("time>?",timestamp_greater,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"timestamp1",kno_any_type,KNO_VOID},
	  {"timestamp2",kno_any_type,KNO_VOID})
...
DEFC_PRIM("time<?",timestamp_lesser,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"timestamp1",kno_any_type,KNO_VOID},
	  {"timestamp2",kno_any_type,KNO_VOID})
...
DEFC_PRIM("future?",futurep,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"timestamp",kno_any_type,KNO_VOID},
	  {"thresh_arg",kno_flonum_type,KNO_VOID})
...
DEFC_PRIM("elapsed-time",elapsed_time,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(0),
	  "**undocumented**",
	  {"arg",kno_any_type,KNO_VOID})
...
{
  enum u8_timestamp_precision prec = xt->u8_prec;
  if (KNO_EQ(slotid,year_symbol))
    ...
    if (xt->u8_tzoff==0)
      ...
      u8_init_xtime(&asgmt,xt->u8_tick,prec,xt->u8_nsecs,0,0);
    ...
    if (prec>=u8_second) {
      ...
      return kno_err(kno_ImpreciseTimestamp,"xtime_get",
	   ...
	   (KNO_EQ(slotid,nanosecond_symbol)) ||
	   (KNO_EQ(slotid,microseconds_symbol)) ||
	   (KNO_EQ(slotid,microsecond_symbol)) ||
	   (KNO_EQ(slotid,milliseconds_symbol)) ||
	   (KNO_EQ(slotid,millisecond_symbol)))
    if (prec>=u8_second) {
      unsigned int nsecs = xt->u8_nsecs;
      if ( (KNO_EQ(slotid,nanoseconds_symbol)) ||
	   (KNO_EQ(slotid,nanosecond_symbol)) ) {
	if (prec>=u8_nanosecond)
	  return KNO_INT(nsecs);
	else if (reterr)
	  return kno_err(kno_ImpreciseTimestamp,"xtime_get",
			 SYM_NAME(slotid),timestamp);
	else return EMPTY;}
      else if ( ( (KNO_EQ(slotid,microseconds_symbol)) && (prec<u8_microsecond) ) ||
		( (KNO_EQ(slotid,microsecond_symbol)) && (prec<u8_microsecond) ) ||
		( (KNO_EQ(slotid,milliseconds_symbol)) && (prec<u8_millisecond) ) ||
		( (KNO_EQ(slotid,millisecond_symbol)) && (prec<u8_millisecond) ) ) {
	if (reterr)
	  return kno_err(kno_ImpreciseTimestamp,"xtime_get",
			 SYM_NAME(slotid),timestamp);
	else return EMPTY;}
...
static int xtime_set(struct U8_XTIME *xt,lispval slotid,lispval value)
{
  ...
  if (KNO_EQ(slotid,year_symbol))
    if (KNO_UINTP(value)) {
      long long ival = FIX2INT(value);
      if (ival<50) ival = ival+2000;
      else if (ival<100) ival = ival+1900;
      else NO_ELSE;
      xt->u8_year = ival;}
    else return kno_reterr(kno_TypeError,"xtime_set",u8_strdup(_("year")),value);
  else if (KNO_EQ(slotid,month_symbol))
    if ((FIXNUMP(value)) && (FIX2INT(value)>0) && (FIX2INT(value)<13))
      xt->u8_mon = FIX2INT(value)-1;
    else return kno_reterr(kno_TypeError,"xtime_set",u8_strdup(_("month")),value);
  else if (KNO_EQ(slotid,date_symbol))
    if ((FIXNUMP(value)) && (FIX2INT(value)>0) && (FIX2INT(value)<32))
    ...
    if ((FIXNUMP(value)) && (FIX2INT(value)>=0) && (FIX2INT(value)<60))
...
static lispval timestamp_get(lispval timestamp,lispval slotid,lispval dflt)
{
  ...
  if (VOIDP(dflt))
    return xtime_get(&(tms->u8xtimeval),slotid,timestamp,1);
  else {
    lispval result = xtime_get(&(tms->u8xtimeval),slotid,timestamp,0);
...
DEFC_PRIM("modtime",modtime_prim,
	  KNO_MAX_ARGS(3)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"slotmap",kno_any_type,KNO_VOID},
	  {"base",kno_any_type,KNO_VOID},
	  {"togmt",kno_any_type,KNO_VOID})
...
DEFC_PRIMN("mktime",mktime_lexpr,
...
DEFC_PRIM("timestring",timestring,
...
DEFC_PRIM("millitime",millitime_prim,
	  KNO_MAX_ARGS(0)|KNO_MIN_ARGS(0),
	  "**undocumented**")
...
DEFC_PRIM("microtime",microtime_prim,
	  KNO_MAX_ARGS(0)|KNO_MIN_ARGS(0),
	  "**undocumented**")
...
DEFC_PRIM("secs->string",secs2string,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"secs",kno_any_type,KNO_VOID},
	  {"prec_arg",kno_any_type,KNO_INT(3)})
...
DEFC_PRIM("secs->short",secs2short,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"secs",kno_any_type,KNO_VOID})
...
DEFC_PRIM("getuuid",getuuid_prim,
	  KNO_MAX_ARGS(2)|KNO_MIN_ARGS(0),
	  "**undocumented**",
	  {"nodeid",kno_any_type,KNO_VOID},
	  {"tptr",kno_any_type,KNO_VOID})
...
DEFC_PRIM("uuid-time",uuidtime_prim,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"uuid_arg",kno_uuid_type,KNO_VOID})
...
DEFC_PRIM("uuid-node",uuidnode_prim,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"uuid_arg",kno_uuid_type,KNO_VOID})
...
DEFC_PRIM("uuid->string",uuidstring_prim,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"uuid_arg",kno_uuid_type,KNO_VOID})
...
DEFC_PRIM("uuid->packet",uuidpacket_prim,
	  KNO_MAX_ARGS(1)|KNO_MIN_ARGS(1),
	  "**undocumented**",
	  {"uuid_arg",kno_uuid_type,KNO_VOID})
...
static void link_local_cprims()
{
  ...
  KNO_LINK_CPRIMN("mktime",mktime_lexpr,timeprims_module);
