  ...
  (fdtype key,struct FD_HASHENTRY **slots,int n_slots,int *n_keys)
{
  ...
  struct FD_HASHENTRY *he=slots[offset];
  if (he == NULL) {
    he=u8_zalloc(struct FD_HASHENTRY);
...
FD_EXPORT fdtype fd_hashtable_keys(struct FD_HASHTABLE *ptr)
{
  ...
  if (unlock) fd_rw_unlock(&ptr->rwlock);
                                    ...
                                    struct FD_KEYVAL *inits)
{
  ...
  ptr->slots=slots=u8_zalloc_n(n_slots,struct FD_HASHENTRY *);
...
static int resize_hashtable(struct FD_HASHTABLE *ptr,int n_slots,int locked)
{
  ...
  if ((!(locked))&&(ptr->uselock)) {
    ...
    unlock=1; }
  {
    struct FD_HASHENTRY **new_slots=u8_zalloc_n(n_slots,fd_hashentry);
    struct FD_HASHENTRY **scan=ptr->slots, **lim=scan+ptr->n_slots;
...
{
  if (readonly) {
    if ((ht->readonly) && (ht->uselock==0))
      return 0;
    else if ((!(ht->readonly)) && (ht->uselock==0)) {
      fd_seterr("Can't lock modifiable which isn't currently locking",
                "fd_lock_hashtable",NULL,FD_VOID);
      return -1;}
    fd_read_lock_struct(ht);
    ht->readonly=1; ht->uselock=0;
    return 1;}
  else {
    if (ht->uselock) return 0;
    ht->uselock=1; ht->readonly=0;
    fd_rw_unlock_struct(ht);
    return 1;}
}
