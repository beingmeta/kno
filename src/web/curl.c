...
{
  lispval urlarg = kno_get_arg(expr,1), url = kno_eval_arg(urlarg,env,_stack);
  ...
  if (KNO_ABORTP(url)) return url;
    ...
    ctype = kno_eval_arg(kno_get_arg(expr,2),env,_stack);
  ...
  if (KNO_ABORTP(ctype)) {
    ...
    curl = kno_eval(kno_get_arg(expr,3),env,_stack);
