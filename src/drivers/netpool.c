...
static int server_supportsp(struct KNO_NETWORK_POOL *np,lispval operation)
{
  ...
  lispval response = kno_dteval(np->pool_connpool,request);
  kno_decref(request);
                                       ...
                                       lispval opts)
{
  ...
  np->pool_connpool=
    u8_open_connpool(spec,kno_dbconn_reserve_default,
                     kno_dbconn_cap_default,kno_dbconn_init_default);
  if (((np)->pool_connpool) == NULL) {
    ...
    kno_decref(pooldata);
    pooldata = kno_dtcall(np->pool_connpool,2,pool_data_symbol,client_id);}
  if (KNO_ABORTP(pooldata)) {
...
static lispval *network_pool_fetchn(kno_pool p,int n,lispval *oids)
{
  ...
  lispval value = kno_dtcall(np->pool_connpool,2,fetch_oids_symbol,oidvec);
  kno_decref(oidvec);
  if (VECTORP(value)) {
