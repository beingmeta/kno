...
static fd_pool open_bigpool(u8_string fname,fdb_flags flags)
{
...
static fdtype read_oid_value(fd_bigpool bp,fd_inbuf in,const u8_string cxt)
{
...
static fdtype read_oid_value_at(fd_bigpool bp,FD_CHUNK_REF ref,const u8_string cxt)
{
...
static fdtype *bigpool_fetchn(fd_pool p,int n,fdtype *oids)
{
  ...
  if (bp->pool_offdata==NULL) {
    ...
    while (i<n) {
      ...
      schedule[i].location=get_chunk_ref(offdata,bp->pool_offtype,off);
...
static int bigpool_storen(fd_pool p,int n,fdtype *oids,fdtype *values)
{
  ...
  u8_log(fdb_loglevel+1,"BigpoolStore",
  ...
  u8_log(fdb_loglevel,"BigpoolStore",
                          ...
                          int n, struct BIGPOOL_SAVEINFO *saveinfo)
{
  ...
  double started=u8_elapsed_time();
  int i=0;
  u8_log(fdb_loglevel+1,"BigpoolFinalize",
