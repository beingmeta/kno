                             ...
                             lispval opts)
{
  ...
  int stream_flags =
    KNO_STREAM_CAN_SEEK | KNO_STREAM_NEEDS_LOCK | KNO_STREAM_READ_ONLY | (0);
...
static lispval bigpool_ctl(kno_pool p,lispval op,int n,kno_argvec args)
{
  ...
  if ((n>0)&&(args == NULL))
    ...
    lispval cname = kno_compression_name(bp->pool_compression);
    if (KNO_ABORTP(cname)) {
      u8_log(LOG_WARN,"BadCompressionType","for bigpool %s, code=0x%x",
	     bp->poolid,((unsigned int)(bp->pool_compression)));
      u8_pop_exception();}
