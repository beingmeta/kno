...
static fd_pool open_file_pool(u8_string fname,fdb_flags flags)
{
...
static fdtype file_pool_fetch(fd_pool p,fdtype oid)
{
  ...
  else if (fp->pool_offdata) data_pos=offget(fp->pool_offdata,offset);
  else {
...
static int file_pool_storen(fd_pool p,int n,fdtype *oids,fdtype *values)
{
  ...
  fd_unlock_stream(stream);
  fd_unlock_pool(fp);
  u8_log(fdb_loglevel,"FilePoolStore",
