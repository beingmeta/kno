...
static int mem_index_commit(fd_index ix)
{
  ...
  if (n_updates>60000)
    u8_log(fdkbase_loglevel,"MemIndex/Commit",
	   "Saving %d updates to %s",n_updates,ix->index_idstring);
  else u8_log(fdkbase_loglevel+1,"MemIndex/Commit",
	      "Saving %d updates to %s",n_updates,ix->index_idstring);
  ...
  u8_log(fdkbase_loglevel+1,"MemIndex/Commit",
  ...
  u8_log(fdkbase_loglevel,"MemIndex/Finished",
...
static fd_index open_mem_index(u8_string file,fdkbase_flags flags)
{
