...
static int libscm_config_set(lispval var,lispval vals,void *d)
{
  if (KNO_STRINGP(vals)) {
	...
	use = kno_mkstring(abspath);
	u8_free(abspath);}
      else kno_incref(vals);
      *lptr = use;
      kno_decref(cur);
      return 1;}}
  ...
  DO_CHOICES(val,vals) {
	...
	while (start) {
	  ...
	  if (skip) {}
	    ...
	    u8_string abspath = u8_abspath(buf,NULL);
	    add_paths = kno_init_pair(NULL,knostring(abspath),add_paths);
	    u8_free(abspath);}
	  if (next)
	    start=next+1;
	  else start=NULL;}}}}
