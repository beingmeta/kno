...
static lispval getopt_evalfn(lispval expr,kno_lexenv env,kno_eval_stack _stack)
{
...
#if 0
static lispval tryopt_evalfn(lispval expr,kno_lexenv env,kno_eval_stack _stack)
{
  lispval opts_arg = kno_get_arg(expr,1);
  if (KNO_VOIDP(opts_arg))
    return kno_err(kno_SyntaxError,"getopt_evalfn/arg1",NULL,expr);
