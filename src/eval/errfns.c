...
DEFC_EVALFN("catcherr",catcherr_evalfn,KNO_EVALFN_DEFAULTS,
...
static lispval catcherr_evalfn(lispval expr,kno_lexenv env,kno_stack _stack)
{
  ...
  KNO_STACK_SET_FLAG(_stack,KNO_STACK_ERR_CATCH,1);
  KNO_STACK_SET_FLAG(_stack,KNO_STACK_ERR_CATCHALL,1);
  lispval value = kno_eval(toeval,env,_stack);
  KNO_STACK_SET_FLAG(_stack,KNO_STACK_ERR_CATCHALL,0);
  KNO_STACK_SET_FLAG(_stack,KNO_STACK_ERR_CATCH,0);
  if (KNO_THROWP(value))
    return value;
...
DEFC_EVALFN("error",error_evalfn,KNO_EVALFN_DEFAULTS,
...
DEFC_EVALFN("irritant",irritant_evalfn,KNO_EVALFN_DEFAULTS,
...
DEFC_EVALFN("onerror",onerror_evalfn,KNO_EVALFN_DEFAULTS,
...
DEFC_EVALFN("report-errors",report_errors_evalfn,KNO_EVALFN_DEFAULTS,
...
DEFC_EVALFN("ignore-errors",ignore_errors_evalfn,KNO_EVALFN_DEFAULTS,
	    "(REPORT-ERRORS *expr* [*errval*]) evaluates *expr*, "
	    "reporting and clearing any errors.\n"
	    "Returns *errval* or #f if any errors occur.")
...
DEFC_PRIM("clear-errors!",clear_errors,
	  ...
	  "Clears all current errors, reporting them to the standard output.")
...
DEFC_EVALFN("dynamic-wind",dynamic_wind_evalfn,KNO_EVALFN_DEFAULTS,
	    "`(dynamic-wind *before* *thunk* *after*)` applies "
	    "*before*, *thunk*, and *after* in order, even if "
	    "*thunk* returns an error (and it's still returned).")
...
DEFC_EVALFN("unwind-protect",unwind_protect_evalfn,KNO_EVALFN_DEFAULTS,
	    "`(unwind-protect *expr* *guards...*)` evaluates *expr* "
	    "and then evaluates *guards...*, even if *expr* returns "
	    "non-locally or encounters an error.")
...
KNO_EXPORT void kno_init_errfns_c()
{
  ...
  KNO_LINK_EVALFN(kno_scheme_module,error_evalfn);
  KNO_LINK_EVALFN(kno_scheme_module,irritant_evalfn);

  KNO_LINK_EVALFN(kno_scheme_module,onerror_evalfn);
  KNO_LINK_EVALFN(kno_scheme_module,catcherr_evalfn);
  KNO_LINK_ALIAS("erreify",catcherr_evalfn,kno_scheme_module);

  KNO_LINK_EVALFN(kno_scheme_module,report_errors_evalfn);
  KNO_LINK_EVALFN(kno_scheme_module,ignore_errors_evalfn);
  KNO_LINK_EVALFN(kno_scheme_module,dynamic_wind_evalfn);
  KNO_LINK_EVALFN(kno_scheme_module,unwind_protect_evalfn);
