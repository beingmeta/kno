...
(define (parse-nmea-sentence line)
  (if (find #\, line)
      (let* ((end (position #\* line))
	     (cksum (if end (read-from-string (subseq line (1+ end))) #f))
...
(define (parse-degree s)
  (if (= (length s) 0) (fail)
    (let ((dot (position #\. s)))
      (if dot (let ((degrees (read-from-string (subseq s 0 (- dot 2))))
		    (minutes (read-from-string (subseq s (- dot 2)
						       0))))
		(+ degrees (/ minutes 60)))
	(read-from-string s)))))
...
(define (read-lat rec i)
  (let ((deg (elt rec i))
    ...
    (if south (- (parse-degree dec))
      (parse-degree deg))))
(define (read-lon rec i)
  (let ((deg (elt rec i))
	(west (member (elt rec (1+ i)) '("W" "w"))))
    (if west (- (parse-degree dec))
      (parse-degree deg))))

(define (readstring s)
  (if (= (length s) 0) (fail) (read-from-string s)))
