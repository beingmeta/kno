...
(defambda (dotighten expr env bound dolex)
  (cond ((ambiguous? expr)
	...
	(else (let* ((head (car expr))
		     (module (and (symbol? head) (wherefrom head env)))
		...
		(cond ((if from (test from '%unoptimized head)
		       ...
		       expr)
		      ((and module (%test module '%subst)
			    (%test (get module '%subst) head))
		       (dotighten ((%get (get module '%subst) head) expr) env bound dolex))
