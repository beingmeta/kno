...
(define (load-translations file)
  (let* ((table (make-hashtable))
    ...
    (cond ((not (pair? entry))
	   ...
	   (set! entry (read in)))
	  ((not (pair? (cdr entry)))
	   (set! inlang (car entry))
	   (set! outlang (cdr entry))
	   (set! entry (read in))))
    (until (eof-object? entry)
      ;; either (native translated) 
      ;; or     (native translated-language translated)
      ;; or     (alt-language alt-native translated-language translated)
      (case (length entry)
	((1) (warn "Invalid translation entry " entry))
	((2) (add! table (cons inlang (first entry))
		   (cons outlang (second entry))))
	((3) (add! table (cons inlang (first entry))
		   (cons (second entry) (third entry))))
	((4) (add! table (cons (first entry) (second entry inlang))
