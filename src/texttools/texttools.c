...
static fdtype md5_prim(fdtype input)
{
  ...
  if (FD_STRINGP(input))
    ...
    digest=u8_md5(out.bytebuf,out.bufwrite-out.bytebuf,NULL);
    u8_free(out.bytebuf);}
  if (digest==NULL)
    return FD_ERROR_VALUE;
...
static fdtype sha1_prim(fdtype input)
{
  ...
  if (FD_STRINGP(input))
    ...
    struct FD_OUTBUF out; FD_INIT_BYTE_OUTBUF(&out,1024);
    fd_write_dtype(&out,input);
    digest=u8_sha1(out.bytebuf,out.bufwrite-out.bytebuf,NULL);
    u8_free(out.bytebuf);}
  if (digest==NULL)
    return FD_ERROR_VALUE;
...
static fdtype sha256_prim(fdtype input)
{
  ...
  if (FD_STRINGP(input))
    ...
    struct FD_OUTBUF out; FD_INIT_BYTE_OUTBUF(&out,1024);
    fd_write_dtype(&out,input);
    digest=u8_sha256(out.bytebuf,out.bufwrite-out.bytebuf,NULL);
    u8_free(out.bytebuf);}
  if (digest==NULL)
    return FD_ERROR_VALUE;
...
static fdtype sha384_prim(fdtype input)
{
  ...
  if (FD_STRINGP(input))
    ...
    struct FD_OUTBUF out; FD_INIT_BYTE_OUTBUF(&out,1024);
    fd_write_dtype(&out,input);
    digest=u8_sha384(out.bytebuf,out.bufwrite-out.bytebuf,NULL);
    u8_free(out.bytebuf);}
  if (digest==NULL)
    return FD_ERROR_VALUE;
...
static fdtype sha512_prim(fdtype input)
{
  ...
  if (FD_STRINGP(input))
    ...
    struct FD_OUTBUF out; FD_INIT_BYTE_OUTBUF(&out,1024);
    fd_write_dtype(&out,input);
    digest=u8_sha512(out.bytebuf,out.bufwrite-out.bytebuf,NULL);
    u8_free(out.bytebuf);}
  if (digest==NULL)
    return FD_ERROR_VALUE;
...
static fdtype hmac_sha1_prim(fdtype key,fdtype input)
{
  ...
  if (FD_STRINGP(input)) {
    ...
    fd_write_dtype(&out,input);
    data=out.bytebuf; data_len=out.bufwrite-out.bytebuf; free_data=1;}
  if (FD_STRINGP(key)) {
    ...
    keydata=out.bytebuf; key_len=out.bufwrite-out.bytebuf; free_key=1;}
