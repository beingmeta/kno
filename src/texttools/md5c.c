...
static unsigned char *MDString(char *string)
{
  MD5_CTX context;
  unsigned char *digest=u8_malloc(16);
...
/* Digests a string and returns the result */
static unsigned char *MDPacket(char *string,int len)
{
  MD5_CTX context;
  unsigned char *digest=u8_malloc(16);
...
FD_EXPORT
fdtype fd_md5(fdtype string)
{
  if (FD_STRINGP(string)) {
    unsigned char *digest=MDString(FD_STRDATA(string));
