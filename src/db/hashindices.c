                                  ...
                                  int dolock)
{
  if (ix->fdx_offdata) {
    FD_CHUNK_REF result;
    if (FD_EXPECT_FALSE(ix->fdb_offtype==FD_B64)) {
...
static fd_index open_hash_index(u8_string fname,int read_only,int consed)
{
  ...
  index->fdb_offtype=(fd_offset_type)
...
static int hash_index_commit(struct FD_INDEX *ix)
{
  ...
  if (fd_acid_files) {
    ...
    fd_dtswrite_4bytes(stream,hx->fdb_xformat);
...
#endif
  if ((buckets) && (hx->fdb_offtype==FD_B64))
    while (i<changed_buckets) {
