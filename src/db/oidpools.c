...
#define POOLFILE_LOCKEDP(op) \
  (U8_BITP((op->stream.flags),FD_DTSTREAM_LOCKED))
...
static int lock_oidpool_file(struct FD_OIDPOOL *fp,int use_mutex)
{
  ...
  else if (U8_BITP((fp->stream.flags),FD_DTSTREAM_READ_ONLY))
    return 0; /* -1 ? */
  else {
...
{
  fd_oidpool fp=(fd_oidpool)p;
  int chunkref_size=get_chunkref_size(fp);
  if (chunkref_size<0) {
  ...
  if (level < 2) {
    ...
    if (LOCK_POOLSTREAM(fp)<0) {
      ...
      fp->offsets=offsets;
      fp->offsets_size=load*chunkref_size;
      UNLOCK_POOLSTREAM(fp);}
    fd_unlock_struct(fp);
    return;}
#else /* HAVE_MMAP */
  ...
  if ( (level >= 3) && (fp->mmap == NULL) ) {
      ...
      if (mmap_size<0) {
        ...
        fd_unlock_struct(fp);
        return;}
      mmapped=mmap(NULL,mmap_size,PROT_READ,MMAP_FLAGS,fp->stream.fd,0);
      if (mmapped) {
        fp->mmap=mmapped;
