...
FD_EXPORT void fd_register_pool_type
  (u8_string name,
   ...
   fd_pool (*opener)(u8_string filename,fdb_flags flags),
...
FD_EXPORT
fd_pool fd_open_pool(u8_string spec,fdb_flags flags)
{
  struct FD_POOL_TYPEINFO *ptype;
  ptype=pool_typeinfo; while (ptype) {
    if (ptype->matcher) {
...
FD_EXPORT void fd_register_index_type
  (u8_string name,
   ...
   fd_index (*opener)(u8_string filename,fdb_flags flags),
   u8_string (*matcher)(u8_string filename,void *),
   void *type_data)
{
...
FD_EXPORT
fd_index fd_open_index(u8_string spec,fdb_flags flags)
{
  struct FD_INDEX_TYPEINFO *ixtype;
  ixtype=index_typeinfo; while (ixtype) {
    if (ixtype->matcher) {
