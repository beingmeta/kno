...
FD_EXPORT void fd_pool_setcache(fd_pool p,int level)
{
  ...
  p->pool_flags=p->pool_flags|FDB_CACHELEVEL_SET;
}
...
void fd_reset_pool_tables(fd_pool p,
                          ...
                          ssize_t locksval)
{
  int read_only=p->pool_read_only;
...
FD_EXPORT int fd_register_pool(fd_pool p)
{
  ...
  if ((capacity>=FD_TOP_POOL_SIZE) && ((p->pool_base)%FD_TOP_POOL_SIZE)) {
    fd_seterr(fd_InvalidPoolRange,"fd_register_pool",u8_strdup(p->pool_cid),FD_VOID);
...
static struct FD_GLUEPOOL *make_gluepool(FD_OID base)
{
  ...
  pool->pool_base=base; pool->pool_capacity=0; pool->pool_read_only=1;
...
FD_EXPORT u8_string fd_pool_id(fd_pool p)
{
  ...
  else if (p->pool_xid!=NULL) return p->pool_xid;
  else if (p->pool_cid!=NULL) return p->pool_cid;
...
static void pool_conflict(fd_pool upstart,fd_pool holder)
{
  if (pool_conflict_handler) pool_conflict_handler(upstart,holder);
    ...
    u8_log(LOG_WARN,_("Pool conflict"),
           ...
           upstart->pool_label,upstart->pool_source,holder->pool_label,holder->pool_source);
...
FD_EXPORT fdtype fd_pool_fetch(fd_pool p,fdtype oid)
{
  ...
  if (p->pool_n_locked)
    if (fd_hashtable_op(&(p->pool_changes),fd_table_replace_novoid,oid,v))
      return v;
...
FD_EXPORT fdtype fd_pool_alloc(fd_pool p,int n)
{
  if (p->pool_read_only)
    ...
    if (FD_CHOICEP(result)) {
      if ((p->pool_flags)&(FDB_LOCKFREE)) {
        fd_hashtable_iterkeys(&(p->pool_cache),fd_table_store,
        ...
        fd_hashtable_iterkeys(&(p->pool_changes),fd_table_store,
                              FD_CHOICE_SIZE(result),FD_CHOICE_DATA(result),
                              FD_EMPTY_CHOICE);
        p->pool_n_locked=p->pool_n_locked+n;}}
    else if (FD_ABORTP(result)) return result;
    else if (FD_EXCEPTIONP(result)) return result;
    else if ((p->pool_flags)&(FDB_LOCKFREE)) {
      fd_hashtable_store(&(p->pool_cache),result,FD_EMPTY_CHOICE);}
    else {
...
FD_EXPORT int fd_pool_prefetch(fd_pool p,fdtype oids)
{
  ...
  if (fd_ipeval_status()) {
    ...
    int nolocks=((p->pool_flags)&(FDB_LOCKFREE))||(locks->fd_n_keys==0);
    ...
    if ((p->pool_n_locked==0) || ((p->pool_flags)&(FDB_LOCKFREE)) ||
        (fd_hashtable_op(&(p->pool_changes),fd_table_replace_novoid,oids,v)==0)) {
...
int fd_prefetch_oids(fdtype oids)
{
  ...
  FD_DO_CHOICES(oid,oids) {
    if ((fdtc)&&(fd_hashtable_probe(&(fdtc->oids),oid))) {}
      ...
      if (p) {
        fdtype v=((p->pool_flags)&(FDB_LOCKFREE))?
          (fd_hashtable_get_noref(&(p->pool_changes),oid,FD_VOID)):
          (FD_VOID);
        if (FD_VOIDP(v)) v=fd_hashtable_get_noref(&(p->pool_cache),oid,FD_VOID);
...
FD_EXPORT int fd_lock_oids(fdtype oids)
{
  ...
  FD_DO_CHOICES(oid,oids) {
    if (FD_OIDP(oid)) {
      ...
      else if (fd_hashtable_probe_novoid(&(p->pool_changes),oid)==0) {
        i=0; while (i<n_pools) if (pools[i]==p) break; else i++;
        if (i>=n_pools) {
...
FD_EXPORT int fd_unlock_oids(fdtype oids,int commit)
{
  ...
  FD_DO_CHOICES(oid,oids) {
    if (FD_OIDP(oid)) {
      ...
      else if (fd_hashtable_probe_novoid(&(p->pool_changes),oid)) {
        i=0; while (i<n_pools) if (pools[i]==p) break; else i++;
        if (i>=n_pools) {
          /* Create a pool entry if neccessary */
          if (i<max_pools) {
...
FD_EXPORT int fd_swapout_oid(fdtype oid)
{
  ...
  if (p==NULL)
    return fd_reterr(fd_AnonymousOID,"SET-OID_VALUE!",NULL,oid);
  else if (p->pool_handler->swapout) return p->pool_handler->swapout(p,oid);
  else if ((!((p->pool_flags)&(FDB_LOCKFREE)))&&
           (fd_hashtable_probe_novoid(&(p->pool_changes),oid)))
    /* Don't swap out locked OIDs */
   return 0;
  else if (!(fd_hashtable_probe_novoid(&(p->pool_cache),oid))) return 0;
  else {
...
FD_EXPORT int fd_swapout_oids(fdtype oids)
{
        ...
        pools[i]=p; toswap[i]=oid; n_pools++;}}}
  {
      ...
      if (p->pool_handler->swapout)
        p->pool_handler->swapout(p,toswap[i]);
      else {
...
FD_EXPORT int fd_finish_oids(fdtype oids,fd_pool poolarg)
{
  if (FD_EMPTY_CHOICEP(oids)) return 0;
    ...
    if (!(p))
      ...
      fd_hashtable locks=&(p->pool_changes);
...
FD_EXPORT int fd_pool_unlock(fd_pool p,fdtype oids,int commit)
{
  ...
  u8_log(fddb_loglevel,"Unlock","Unlocking %d oids in pool %s",
         FD_CHOICE_SIZE(oids),p->pool_cid);
  if (p->pool_handler->unlock==NULL) {
    if ((p->pool_flags)&(FDB_LOCKFREE)) {
      return 0;}
    else {
      fd_seterr(fd_NoLocking,"fd_pool_lock",u8_strdup(fd_pool_id(p)),FD_VOID);
  ...
  if (FD_CHOICEP(oids)) {
    ...
    struct FD_CHOICE *oidc=fd_alloc_choice(FD_CHOICE_SIZE(oids));
    fdtype *oidv=(fdtype *)FD_XCHOICE_DATA(oidc), *write=oidv;
    u8_log(fddb_loglevel+1,"Unlock","Cleaning up locks table for %s",p->pool_cid);
    FD_DO_CHOICES(o,oids)
    ...
    if (p->pool_handler->unlock==NULL) {
      fd_seterr(fd_CantLockOID,"fd_pool_unlock",
                u8_strdup(p->pool_cid),oids);
      return -1;}
    u8_log(fddb_loglevel+1,"Unlock","Running custom unlock for %s",p->pool_cid);
    needy=fd_init_choice(oidc,n,NULL,FD_CHOICE_ISATOMIC);
    retval=p->pool_handler->unlock(p,needy);
    if (retval<0) {
      fd_decref(needy);
      return -1;}
    else if (retval) {
    ...
    int retval=p->pool_handler->unlock(p,oids);
  ...
  if (locks->fd_n_keys==0) {
    ...
    if (rv>0)
      u8_log(fddb_loglevel+1,fd_PoolCommit,
             "####### Committed one OIDs in %s",p->pool_cid);
    else if (rv==0)
      u8_log(fddb_loglevel+1,fd_PoolCommit,
             "####### No modified OIDs in %s",p->pool_cid);
    else {}
    return rv;}
                             ...
                             fd_pool_commit_flags flags)
{
  ...
  if ((FD_EMPTY_CHOICEP(oids))||(p->pool_changes.fd_n_keys==0)) {
    fd_rw_unlock_struct(&(p->pool_changes));
    return 0;}
  ...
  if (retval<0) {
    u8_log(LOGCRIT,fd_PoolCommitError,
           ...
           errno,u8_strerror(errno),p->pool_cid);
    u8_seterr(fd_PoolCommitError,"fd_pool_commit",u8_strdup(p->pool_cid));
    if (unlock) restore_locks(locks,&writes);
    return retval;}
  else if (writes.len) {
    u8_log(fddb_loglevel,fd_PoolCommit,
           "####### Saved %d OIDs to %s in %f secs",
           writes.len,p->pool_cid,u8_elapsed_time()-start_time);
    last_report=u8_elapsed_time();
    if (unlock) {
      int unlocked=p->pool_handler->unlock(p,writes.choice);
      if (unlocked<0) {
        u8_log(LOG_CRIT,fd_PoolCommit,
               "Error unlocking %d OIDs from %s after %f secs",
               writes.len,p->pool_cid,
  ...
  if (writes.len)
    u8_log(fddb_loglevel,fd_PoolCommit,
           ...
           writes.len,p->pool_cid,locks->fd_n_keys,
...
static int just_unlock(fd_pool p,fdtype oids,fd_pool_commit_flags flags)
{
  ...
  if (FD_EMPTY_CHOICEP(oids))
    ...
    int unlocked=p->pool_handler->unlock(p,choice);
    ...
    return choice_size;}
  else if ((unlock)&&(!((p->pool_flags)&(FDB_LOCKFREE)))) {
...
static int commit_one_oid(fd_pool p,fdtype oid,fd_pool_commit_flags flags)
{
  ...
  if (!(savep(value,flags))) {
    ...
    retcode=p->pool_handler->storen(p,1,oidv,values);
    if (retcode<0) {
      u8_seterr(fd_PoolCommitError,"fd_pool_commit",u8_strdup(p->pool_cid));
      fd_decref(value);
      return retcode;}
...
{
  u8_log(fddb_loglevel,"PoolDB","Swapping out pool %s",p->pool_cid);
  if (p->pool_handler->swapout) {
    p->pool_handler->swapout(p,FD_VOID);
    u8_log(fddb_loglevel+1,"PoolDB",
           "Finished custom swapout for pool %s",p->pool_cid);}
  if (p) {
    if ((p->pool_flags)&(FD_STICKY_CACHESIZE))
      fd_reset_hashtable(&(p->pool_cache),-1,1);
    else fd_reset_hashtable(&(p->pool_cache),fd_pool_cache_init,1);}
  if (p) {
...
FD_EXPORT fdtype fd_pool2lisp(fd_pool p)
{
  if (p==NULL)
    ...
    return fd_err(fd_UnregisteredPool,"fd_pool2lisp",p->pool_cid,FD_VOID);
...
FD_EXPORT fd_pool fd_find_pool_by_cid(u8_string cid)
{
  ...
  while (i < 1024)
    if (fd_top_pools[i] == NULL) i++;
      ...
      if ((fd_top_pools[i]->pool_cid) &&
          ((strcmp(canonical,fd_top_pools[i]->pool_cid)) == 0)) {
        u8_free(canonical);
      ...
      while (j<gp->n_subpools)
        if ((subpools[j]) &&
            (subpools[j]->pool_cid) &&
            (strcmp(cid,subpools[j]->pool_cid)==0)) {
          u8_free(canonical);
          return subpools[j];}
        else j++;}
  u8_free(canonical);
...
FD_EXPORT fdtype fd_find_pools_by_cid(u8_string cid)
{
  ...
  while (i < 1024)
    if (fd_top_pools[i] == NULL) i++;
      ...
      fd_pool *subpools=gp->subpools; int j=0;
      while (j<gp->n_subpools)
        if ((subpools[j]->pool_cid) && (strcmp(canonical,subpools[j]->pool_cid)==0)) {
  ...
  if (retval<0)
    if (data) {
      u8_log(LOG_CRIT,"POOL_COMMIT_FAIL",
             "Error when committing pool %s",p->pool_cid);
      return 0;}
    else return -1;
  else return 0;
}
...
{
  int *count=(int *)ptr;
  *count=*count+p->pool_cache.fd_n_keys;
  return 0;
}
                            ...
                            u8_string source,u8_string cid)
{
  ...
  p->pool_serialno=-1; p->pool_cache_level=-1; p->pool_read_only=1;
  ...
  p->pool_max_adjuncts=0; p->pool_n_adjuncts=0; p->pool_adjuncts=NULL;
  p->oid_handlers=NULL;
  p->pool_n_locked=0;
  p->pool_handler=h;
  p->pool_source=u8_strdup(source); p->pool_cid=u8_strdup(cid); p->pool_xid=NULL;
  p->pool_label=NULL; p->pool_prefix=NULL; p->pool_namefn=FD_VOID;
}
...
static int unparse_pool(u8_output out,fdtype x)
{
  ...
  sprintf(addrbuf,"@%x/%x+0x%x",FD_OID_HI(p->pool_base),FD_OID_LO(p->pool_base),p->pool_capacity);
  if (p->pool_label)
    if ((p->pool_xid) && (strcmp(p->pool_source,p->pool_xid)))
      u8_printf(out,"#<POOL %s %s #!%lx \"%s\" \"%s|%s\">",
                type,addrbuf,x,p->pool_label,p->pool_source,p->pool_xid);
    ...
    if ((p->pool_xid) && (strcmp(p->pool_source,p->pool_xid)))
      u8_printf(out,"#<POOL %s %s #!%lx \"%s|%s\">",
                type,addrbuf,x,p->pool_source,p->pool_xid);
  ...
  if (p->pool_label)
    if ((p->pool_xid) && (strcmp(p->pool_source,p->pool_xid)))
      u8_printf(out,"#<POOL %s 0x%lx \"%s\" \"%s|%s\">",
                type,x,p->pool_label,p->pool_source,p->pool_xid);
    else u8_printf(out,"#<POOL %s 0x%lx \"%s\" \"%s\">",type,x,p->pool_label,p->pool_source);
  else if (p->pool_source)
    if ((p->pool_xid) && (strcmp(p->pool_source,p->pool_xid)))
      u8_printf(out,"#<POOL %s 0x%lx \"%s|%s\">",type,x,p->pool_source,p->pool_xid);
                                  ...
                                  unsigned int noswap)
{
  ...
  mp->pool_load=load; mp->pool_read_only=0; mp->noswap=noswap;
  u8_init_mutex(&(mp->pool_lock));
  mp->pool_flags=mp->pool_flags|(FDB_LOCKFREE);
  ...
  if ((mp->pool_load+n)>=mp->pool_capacity)
    ...
    if ((mp->pool_load+n)>=mp->pool_capacity) {
      ...
      while (i<n) {
        ...
        i++;}
      mp->pool_load=mp->pool_load+n; mp->pool_n_locked=mp->pool_n_locked+n;
      u8_unlock_mutex(&(mp->pool_lock));
...
static int mempool_lock(fd_pool p,fdtype oids)
{
  struct FD_MEMPOOL *mp=(fd_mempool)p;
  u8_lock_mutex(&(mp->pool_lock));
  mp->pool_n_locked=mp->pool_n_locked+FD_CHOICE_SIZE(oids);
  return 1;
}
...
FD_EXPORT int fd_clean_mempool(fd_pool p)
{
  if (p->pool_handler!=&mempool_handler)
    ...
    return p->pool_cache.fd_n_keys+p->pool_changes.fd_n_keys;}
}
                        ...
                        fdtype state)
{
  if (!(FD_EXPECT_TRUE(FD_APPLICABLEP(fetchfn)))) {
    ...
    if (!(FD_VOIDP(savefn))) xp->pool_read_only=0;
    ...
    if ((FD_VOIDP(lockfn))||(FD_FALSEP(lockfn)))
      xp->pool_flags=xp->pool_flags|FDB_LOCKFREE;
    return (fd_pool)xp;}
}
...
static int extpool_lock(fd_pool p,fdtype oids)
{
  ...
  if (FD_APPLICABLEP(xp->lockfn)) {
    ...
    return -1;}
  else if ((xp->pool_flags)&(FDB_LOCKFREE)) {
    return 0;}
  else {
    FD_DO_CHOICES(oid,oids) {
...
static void recycle_raw_pool(struct FD_CONS *c)
{
  ...
  if (p->pool_xid) u8_free(p->pool_xid);
        ...
        if (p->pool_cid)
          if (p->pool_xid)
            sprintf(oid_info_buf,"@%x/%x in %s from %s = %s = %s",
                    hi,lo,p->pool_label,p->pool_source,p->pool_cid,
                    p->pool_xid);
      ...
      if (p->pool_cid)
        if (p->pool_xid)
          sprintf(oid_info_buf,"@%x/%x from %s = %s = %s",
                  hi,lo,p->pool_source,p->pool_cid,p->pool_xid);
        else sprintf(oid_info_buf,"@%x/%x from %s = %s",
                     hi,lo,p->pool_source,p->pool_cid);
      else sprintf(oid_info_buf,"@%x/%x from %s",hi,lo,p->pool_source);
    else if (p->pool_cid)
      if (p->pool_xid)
        sprintf(oid_info_buf,"@%x/%x from %s = %s",
                hi,lo,p->pool_cid,p->pool_xid);
      else sprintf(oid_info_buf,"@%x/%x from %s",hi,lo,p->pool_cid);
