...
void fd_reset_pool_tables(fd_pool p,
                          ...
                          ssize_t locksval)
{
  int read_only=U8_BITP(p->pool_flags,FDB_READ_ONLY);
  ...
  if (locks->table_n_keys==0) {
    ...
    if (writes.len) {
      u8_log(fddb_loglevel,"PoolCommit",
             "####### Saving %d/%d OIDs in %s",writes.len,p->pool_cid);
      init_cache_level(p);
...
FD_EXPORT int fd_unlock_oids(fdtype oids_arg,fdb_unlock_flag flags)
{
  switch (flags) {
  ...
  while (i < 1024)
    if (fd_top_pools[i] == NULL) i++;
      ...
      while (j<gp->n_subpools)
        if ((subpools[j]) &&
            (subpools[j]->pool_cid) &&
            (strcmp(canonical,subpools[j]->pool_cid)==0)) {
          u8_free(canonical);
          return subpools[j];}
        else j++;}
  u8_free(canonical);
