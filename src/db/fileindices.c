...
static fd_index open_file_index(u8_string fname,int read_only,int consed)
{
  ...
  if (magicno == FD_FILE_INDEX_MAGIC_NUMBER) index->fd_hashv=1;
  else if (magicno == FD_MULT_FILE_INDEX_MAGIC_NUMBER) index->fd_hashv=2;
  else if (magicno == FD_MULT_FILE3_INDEX_MAGIC_NUMBER) index->fd_hashv=3;
  else {
    fd_seterr3(fd_NotAFileIndex,"open_file_index",u8_strdup(fname));
    u8_free(index);
    return NULL;}
  index->fd_offsets=NULL; index->fd_read_only=read_only;
...
static void file_index_setcache(fd_index ix,int level)
{
  ...
  if (level == 2)
    if (fx->fd_offsets) return;
      ...
      if ((newmmap==NULL) || (newmmap==((void *)-1))) {
        u8_log(LOG_CRIT,u8_strerror(errno),
               "file_index_setcache:mmap %s",fx->fd_source);
    ...
    if (fx->fd_offsets == NULL) return;
      ...
      if (retval<0) {
        u8_log(LOG_CRIT,u8_strerror(errno),
               "file_index_setcache:munmap %s",fx->fd_source);
        fx->fd_offsets=NULL; errno=0;}
#else
...
FD_FASTOP unsigned int file_index_hash(struct FD_FILE_INDEX *fx,fdtype x)
{
  ...
  default:
    u8_raise(_("Bad hash version"),"file_index_hash",fx->fd_cid);}
  /* Never reached */
  return -1;
}
    ...
    while (keypos) {
      ...
      if (FD_EXPECT_FALSE((n_vals==0) && (val_start)))
        u8_log(LOG_CRIT,fd_FileIndexError,
               "file_index_fetch %s",u8_strdup(ix->fd_cid));
      ...
      if (FDTYPE_EQUAL(key,thiskey)) {
          ...
          while (next_pos) {
            ...
            if (FD_EXPECT_FALSE(i>=n_vals))
              u8_raise(_("inconsistent file index"),
                       "file_index_fetch",u8_strdup(ix->fd_cid));
        ...
        return fd_err(fd_FileIndexOverflow,"file_index_fetch",
                      u8_strdup(fx->fd_source),thiskey);}
      else {
        n_probes++;
        fd_decref(thiskey);
...
static int file_index_fetchsize(fd_index ix,fdtype key)
{
  ...
  fd_lock_struct(fx);
  {
    ...
    unsigned int n_probes=0;
    unsigned int probe=hashval%(fx->fd_n_slots),
      chain_width=(hashval%(fx->fd_n_slots-2))+1;
    unsigned int *offsets=fx->fd_offsets;
    unsigned int keypos=
      ((offsets) ? (offget(offsets,probe)) : (get_offset(fx,probe)));
    while (keypos) {
      ...
      if (FDTYPE_EQUAL(key,thiskey)) {
        ...
        return fd_err(fd_FileIndexOverflow,
                      ...
                      u8_strdup(fx->fd_source),FD_VOID);}
...
static struct FD_KEY_SIZE *file_index_fetchsizes(fd_index ix,int *n)
{
  ...
  while (i < n_keys) {
    ...
    key=fd_dtsread_dtype(stream);
    sizes[i].fdks_key=key; sizes[i].fdks_nvals=size;
    i++;}
  *n=n_keys;
  u8_free(offsets);
  return sizes;
                        ...
                        fdtype *values)
{
  ...
  while (i < n) {
    ...
    if ((offsets == NULL) && (schedule[i].fs.probe<0)) {
      ...
      schedule[i].fs.filepos=(fd_off_t)(fd_dtsread_4bytes(&(fx->fd_stream)));
      ...
      n_values=fd_dtsread_4bytes(&(fx->fd_stream));
      vpos=(fd_off_t)fd_dtsread_4bytes(&(fx->fd_stream));
      key=fd_dtsread_dtype(&(fx->fd_stream));
      ...
      fd_setpos(&(fx->fd_stream),vpos); val=fd_dtsread_dtype(&(fx->fd_stream));
      if (FD_ABORTP(val)) return fd_interr(val);
      FD_ADD_TO_CHOICE(values[index],val);
      vpos=(fd_off_t)fd_dtsread_4bytes(&(fx->fd_stream));
      while (vpos==1) {
        val=fd_dtsread_dtype(&(fx->fd_stream));
        if (FD_ABORTP(val)) return fd_interr(val);
        FD_ADD_TO_CHOICE(values[index],val);
        vpos=(fd_off_t)fd_dtsread_4bytes(&(fx->fd_stream));}
...
static fdtype *fetchn(struct FD_FILE_INDEX *fx,int n,fdtype *keys,int lock_adds)
{
  ...
  int i=0, schedule_size=0, init_schedule_size; while (i < n) {
    fdtype key=keys[i], cached=fd_hashtable_get(&(fx->fd_cache),key,FD_VOID);
    if (redundantp(fx,key))
      ...
      if (ksched->filepos)
        if (lock_adds)
          values[i]=fd_hashtable_get(&(fx->fdx_adds),key,FD_VOID);
        else values[i]=fd_hashtable_get_nolock(&(fx->fdx_adds),key,FD_VOID);
      else if (lock_adds)
        values[i]=fd_hashtable_get(&(fx->fdx_adds),key,FD_EMPTY_CHOICE);
      else values[i]=fd_hashtable_get_nolock(&(fx->fdx_adds),key,FD_EMPTY_CHOICE);
      i++; schedule_size++;}
  ...
  while (schedule_size>0) {
    ...
    qsort(schedule,schedule_size,sizeof(struct FETCH_SCHEDULE),sort_by_filepos);}
  /* Note that we should now look at fx->fdx_edits and integrate any changes,
                         ...
                         unsigned int *offsets)
{
  ...
  struct FD_DTYPE_STREAM *stream=&(fx->fd_stream);
  unsigned int pos_offset=fx->fd_n_slots*4, chain_length=0;
  ...
  while (max>0) {
    ...
    if (max==i)
      if (chain_length>256) {
        ...
        return fd_reterr(fd_FileIndexOverflow,"fetch_keydata",u8_strdup(fx->fd_cid),FD_VOID);}
      else chain_length++;
    else chain_length=0;
    max=i;}
  if (offsets == NULL) u8_free(reserved.slotnos);
...
static int commit_edits(struct FD_FILE_INDEX *f,struct KEYDATA *kdata)
{
  ...
  if (f->fdx_edits.fd_n_keys==0) return 0;
  dropkeys=u8_alloc_n(f->fdx_edits.fd_n_keys,fdtype);
  scan=f->fdx_edits.fd_buckets; limit=scan+f->fdx_edits.fd_n_buckets;
  while (scan < limit)
    if (*scan) {
      ...
      while (kvscan<kvlimit) {
        ...
        if ((FD_PAIRP(key)) &&
          ...
          fdtype cached=fd_hashtable_get(&(f->fd_cache),FD_CDR(key),FD_VOID);
          if (!(FD_VOIDP(cached))) {
...
static int file_index_commit(struct FD_INDEX *ix)
{
  ...
  fd_write_lock_struct(&(ix->fdx_adds));
  fd_write_lock_struct(&(ix->fdx_edits));
  fd_lock_struct(fx);
  fd_dts_start_write(stream);
  /* Get the current offsets from the index */
#if HAVE_MMAP
  if (fx->fd_offsets) {
    int i=0, n=fx->fd_n_slots;
    /* We have to copy these if they're MMAPd, because
       we can't modify them (while updating) otherwise.  */
    new_offsets=u8_alloc_n((fx->fd_n_slots),unsigned int);
    gc_new_offsets=1;
    while (i<n) {
      new_offsets[i]=offget(fx->fd_offsets,i); i++;}}
#else
  if (fx->fd_offsets) {
    new_offsets=fx->fd_offsets;
    fd_setpos(stream,8);
    fd_dtsread_ints(stream,fx->fd_n_buckets,new_offsets);}
#endif
  {
    fd_off_t filepos;
    int n_adds=ix->fdx_adds.fd_n_keys, n_edits=ix->fdx_edits.fd_n_keys;
    ...
    struct FD_HASH_BUCKET **scan=ix->fdx_adds.fd_buckets, **limit=scan+ix->fdx_adds.fd_n_buckets;
    while (scan < limit)
      if (*scan) {
        struct FD_HASH_BUCKET *e=*scan; int n_keyvals=e->fd_n_entries;
        struct FD_KEYVAL *kvscan=&(e->fd_keyval0), *kvlimit=kvscan+n_keyvals;
    ...
    scan=ix->fdx_adds.fd_buckets; limit=scan+ix->fdx_adds.fd_n_buckets;
    i=0; while (scan < limit)
      if (*scan) {
        ...
        while (kvscan<kvlimit) {
          ...
          filepos=filepos+
            write_values(&(fx->fd_stream),kvscan->fd_keyval,kdata[i].pos,
                         &new_values);
    ...
    if (fd_acid_files) {
      ...
      if (new_offsets) {
        ...
        fd_movepos(stream,-(4*(fx->fd_n_slots)));
        retval=ftruncate(stream->fd_fileno,end-(4*(fx->fd_n_slots)));
        if (retval<0)
...
{
  struct FD_DTYPE_STREAM *stream=&(fx->fd_stream);
  int i=0, n_slots=fx->fd_n_slots; unsigned int magic_no;
  fd_endpos(stream);
  while (i<n_slots) {
...
static int recover_file_index(struct FD_FILE_INDEX *fx)
{
  /* This reads the offsets vector written at the end of the file
     during commitment. */
  int i=0, len=fx->fd_n_slots, retval=0; fd_off_t new_end;
  unsigned int *offsets=u8_malloc(4*len), magic_no;
  struct FD_DTYPE_STREAM *s=&(fx->fd_stream);
...
static void file_index_close(fd_index ix)
{
  ...
  if (fx->fd_offsets) {
    ...
    if (retval<0) {
      u8_log(LOG_CRIT,u8_strerror(errno),
             "[%d:%d] file_index_close:munmap %s",retval,errno,fx->fd_source);
      errno=0;}
#else
    u8_free(fx->fd_offsets);
#endif
