    ...
    (div (class "conceptsummary")
	  ...
	  (anchor concept
		  (do-choices (word (get concept language) i)
		    (if (> i 0) (xmlout " . "))
		    (hashset-add! seen word)
		    (set! j (1+ j))
		    (wordform word concept))
		  (do-choices (lang languages)
		    (do-choices (word (reject (get concept lang) seen) i)
		      (if (> i 0) (xmlout " . "))
		      (hashset-add! seen word)
		      (set! j (1+ j))
		      (wordform word concept)))
		  (do-choices (name (reject (get concept 'names) seen) i)
		    (if (> j 5)
			(set! j (+ j 1))
			(begin (if (> i 0) (xmlout " . "))
	  ...
	  (when (>= j 5)
	    (xmlout
	     " "
	     (hideshow (oid2id concept "WDS") "show more" "show less")
	     " ")
	    (span ((id (oid2id concept "WDS"))
		   (style "display: none;"))
	      (do-choices (name (reject (get concept 'names) seen) i)
		(if (> i 0) (xmlout " . "))
		(hashset-add! seen name)
