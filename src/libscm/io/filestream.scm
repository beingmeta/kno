...
(use-module '{fifo varconfig ezrecords text/stringfmts reflection 
	      archivetools bugjar bugjar/html logger})
  ...
  (let ((port (filestream-port stream))
	...
	(let ((item (readfn port)))
	  (when item 
	    ...
	    (set! count (1+ count)))))
      (set-filestream-itemcount! stream (+ count (filestream-itemcount stream)))
      (set-filestream-filepos! stream (getpos port))
