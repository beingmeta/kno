...
(define (engine/readfile/fillfn count loop-state)
  ...
  (let* ((opts (get loop-state 'opts))
	 ...
	 (posfn (getopt opts 'posfn #f))
	 (instream (get loop-state 'instream))
	 (result (make-vector count))
	 (written 0)
	 (item #f)
	 (pos #f))
    (when (and posfn (not (applicable? posfn))) (set! posfn getpos))
    (debug%watch "engine/readfile/fillfn" count readfn instream)
    (dotimes (i count)
      (when posfn (set! pos (posfn instream)))
      ...
      (cond ((eof-object? item)
	    ...
	    (item
	     (if pos
		 (vector-set! result written (cons pos item))
