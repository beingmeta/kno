...
(in-module 'knodb/actions/copykeys)

(module-export! '{copykeys main})
...
(define (do-copykeys in head-file tail-file tailcount)
  (let* ((restart (config 'restart #f config:boolean))
    ...
    (when (and head-file (file-exists? (glom head-file ".part")))
      (cond (restart 
	    ...
	    (else (logerr |PartialFile|
		  ...
		  (error |PartialFile| (write (glom head-file ".part"))))))
    (let* ((head (index/ref head-file
	   ...
	   (opts (frame-create #f
		   ...
		   'direct2disk (config 'direct2disk {} config:boolean)
		   'maxcount (config 'maxcount {})
		   'mincount (config 'mincount {}))))
      (config! 'appid (glom "copykeys(" (basename in) ")"))
      (unless (index/copy! in head opts)
	(error "Pack index failed"))
      (close-index head)
      (when tail (close-index tail)))))

(define (copykeys (in #f) (head) 
  ...
  (if (and (string? in) (file-exists? in))
      (do-copykeys in head tail tailcount)
      (usage)))
