...
(use-module '{kno/reflect logger logctl stringfmts mttools fifo engine})
...
(defambda (kindex-simple-copier buckets batch-state loop-state task-state)
...
(define (kindex/copy-keys! index output opts (rare))
  (let* ((started (elapsed-time))
    ...
    (engine/run (if (fail? {rare unique})
		    kindex-simple-copier
		    kindex-copier)
	buckets  
