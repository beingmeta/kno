...
(define (hashindex/mapkeys mapfn index (opts #f))
  (let* ((n-buckets (indexctl index 'hash))
    ...
    (let ((loopfn (lambda (span batch-state loop-state task-state)
		    (mapfn span batch-state loop-state task-state)))
	  (opts (cons `#[loop ,loop-init] opts)))
      (store! loop-init 'index index)
      (engine/run loopfn spans opts))))

(define (update-histogram span batch-state loop-state task-state)
...
(define (update-slotkeyset span batch-state loop-state task-state)
  (let* ((index (get loop-state 'index))
	 (hashset (get loop-state 'keyset))
	 (slotids (get loop-state 'slotids))
	 (info (indexctl index 'buckets (car span) (cdr span)))
	 (batchset (make-hashset (|| info)))
	 (key #f))
    (logwarn |Scanning| "From span " span " got " (|| info) " key entries") 
    (do-choices (keyinfo info)
      (set! key (get keyinfo 'key))
      (when (and (pair? key) (overlaps? (car key) slotids))
	(hashset-add! batchset key)))
    (logwarn |Merging| hashset " <== " batchset)
    (hashset/merge! hashset batchset)))
