	...
	(let ((ctype (or ctype (guess-mimetype content opts)))
	  ...
	  (prog1
	      (cond ((and (string? dest) (string-starts-with? dest {"http:" "https:"}))
		    ...
		    ((and (pair? dest) (string? (cdr dest)))
		     (cond ((string? (car dest))
			    (gp/save! (mkpath (car dest) (cdr dest)) content ctype charset opts encoding))
			   ((handles? (car dest) 'gpath/save!)
			    (kno/dispatch (car dest) 'gpath/save! (cdr dest) content ctype opts))
			   (else (irritant (car dest) |NoSaveMethod|))))
		    ((handles? dest 'gpath/save!)
		     (kno/dispatch dest 'gpath/save! #f content ctype opts))
		    (else  (irritant dest |NoSaveMethod|)) )
...
(define-init s3-converter (lambda (s) (convert-s3-uri s)))
(unless (position s3-converter gpath-uri-handlers)
  (set! gpath-uri-handlers (cons s3-converter gpath-uri-handlers)))
