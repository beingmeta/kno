...
(module-export! '{random-oidvec random-stringvec})
...
(define (random-packet (len))
  (default! len (random 100))
...
(define (random-string (len #f))
  (default! len (if (zero? (random 25)) (random 100) (random 15)))
  (cond ((not (number? len))
	 (set! len (if (zero? (random 25)) (random 100) (random 15))))
	((< len 0) (set! len (random (- len)))))
  (stringout
    (dotimes (i len)
      (if (zero? (random 10))
...
(define (generate-baseoids n)
  (let ((result (make-vector n)))
    (dotimes (i n)
      (vector-set! result i (make-oid (random (* 1024 1024))
				      (* (random 512) (* 1024 1024)))))
    result))

(define (random-oidvec n (baseoids #f))
  (let* ((baseoids
	  (cond ((ambiguous? baseoids) (choice->vector baseoids))
		((vector? baseoids) baseoids)
		((integer? baseoids) (generate-baseoids baseoids))
		(else (choice->vector random-super-pools))))
	 (n-baseoids (length baseoids))
	 (result (make-vector n)))
    (dotimes (i n)
      (vector-set! result i (oid-plus (elt baseoids (random n-baseoids))
				      (random (* 1024 1024 512)))))
    result))
(define (random-stringvec n (uselen #f))
  (when (< n 0) (set! n (random (- n))))
  (let* ((result (make-vector n)))
    (dotimes (i n)
      (vector-set! result i (random-string uselen)))
    result))
