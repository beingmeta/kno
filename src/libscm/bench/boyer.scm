...
(define (add-lemma term)
  (cond ((and (pair? term)
         ...
         (pput (car (cadr term))
	              (quote lemmas)
        	      (cons term (pget (car (cadr term)) (quote lemmas)))))
...
(define (apply-subst alist term)
  (cond ((not (pair? term))
         (cond ((begin (set! temp-temp (classq term alist))
                       temp-temp)
                (cdr temp-temp))
               (else term)))
        (else (cons (car term)
...
(define (one-way-unify1 term1 term2)
  (cond ((not (pair? term2))
         (cond ((begin (set! temp-temp (classq term2 unify-subst))
                       temp-temp)
  ...
  (add-lemma-lst
   (quote ((equal (compile form)
           ...
           (equal (mc-flatten x y)
                  (append (flatten x)
                          y))
           (equal (clmember x (append a b))
                  (or (clmember x a)
                      (clmember x b)))
           (equal (clmember x (reverse y))
                  (clmember x y))
           (equal (length (reverse x))
                  (length x))
           (equal (clmember a (intersect b c))
                  (and (clmember a b)
                       (clmember a c)))
           (equal (nth (zero)
                       i)
...
(define benchfn boyer/test)

(module-export! '{boyer/test benchfn})

;;(run-benchmark "Boyer" (lambda () (boyer)))
