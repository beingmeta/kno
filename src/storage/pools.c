...
lispval get_oid_value(lispval oid,kno_pool p)
{
  ...
  if (knocache) {
    value = ((knocache->oids.table_n_keys)?
	     (kno_hashtable_get(&(knocache->oids),oid,VOID)):
	     (VOID));
  ...
  if (knocache) kno_hashtable_store(&(knocache->oids),oid,value);
  return value;
}
...
{
  KNOCACHE *knocache = ((KNO_USE_THREADCACHE)?(kno_threadcache):(NULL));
  struct KNO_HASHTABLE *oidcache = ((knocache!=NULL)?(&(knocache->oids)):(NULL));
  ...
  if (kno_ipeval_status()) {
	...
	*write++=o; n_locked++;}
	...
	int j = 0; while (j<n) {
	  ...
	  if (knocache) kno_hashtable_op(&(knocache->oids),kno_table_store,oid,v);
	...
	int j = 0; while (j<n) {
	  ...
	  modify_readonly(v,1);
	  j++;}
	if (knocache) kno_hashtable_iter(oidcache,kno_table_store,n,oidv,values);
    ...
    if (knocache) kno_hashtable_op(&(knocache->oids),kno_table_store,oids,v);
    kno_decref(v);
    return 1;}
  else return 0;
}
...
{
  if (RARELY( (p->pool_flags) & (KNO_POOL_PREALLOC) ))
    return kno_err(kno_PreallocatedPool,"kno_pool_alloc",p->poolid,KNO_VOID);
  else if (RARELY( (p->pool_flags) & (KNO_STORAGE_READ_ONLY) ))
    return kno_err(kno_ReadOnlyPool,"kno_pool_alloc",p->poolid,KNO_VOID);
  else {
			      ...
			      lispval opts)
{
  ...
  if (KNO_SLOTMAPP(metadata)) {
    ...
    if (kno_test(metadata,KNOSYM(prealloc),KNO_VOID))
      p->pool_flags |= KNO_POOL_PREALLOC;
...
static void display_pool(u8_output out,kno_pool p,lispval lp)
{
  ...
  int is_prealloc = ( (p->pool_flags) & (KNO_POOL_PREALLOC) );
  strcpy(addrbuf,"@");
