...
(use-module '{fdweb xhtml/brico texttools reflection domutils})
...
(define (concept->rdf concept
  ...
  (let ((languages (get-languages)))
	...
	(xmlout "\n\t")
	(xmlelt "rdfs:subclassOf" 'resource (get-rdf-ref g local)))
	...
	(let* ((value (get concept slot))
	  ...
	  (when (exists? value)
		...
		(xmlout
		  (if (oid? value)
		      (xmlelt label 'resource (get-rdf-ref v local))
		      (xmlblock label () v))
		  "\n\t"))
	    (xmlclose fakenode))))
      "\n")))
...
(defambda (->rdf f (opts #[]))
  (let* ((local (choice->hashset f))
	...
	(cond ((test concept 'type 'individual)
	       (individual->rdf concept local slots))
	      (else (concept->rdf concept local slots)))
	"\n"))
    (xmlclose fakenode)
    (void)))
