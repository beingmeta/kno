...
(SECTION 4 1 2)
(applytester '(quote a) 'quote (quote 'a))
(applytester '(quote a) 'quote ''a)
(SECTION 4 1 3)
...
(SECTION 4 1 5)
(applytester 'yes 'if (if (> 3 2) 'yes 'no))
(applytester 'no 'if (if (> 2 3) 'yes 'no))
(applytester '1 'if (if (> 3 2) (- 3 2) (+ 3 2)))
(SECTION 4 1 6)
(define x 2)
(applytester 3 'define (+ x 1))
(set! x 4)
(applytester 5 'set! (+ x 1))
(SECTION 4 2 1)
(applytester 'greater 'cond (cond ((> 3 2) 'greater)
			   ((< 3 2) 'less)))
(applytester 'equal 'cond (cond ((> 3 3) 'greater)
			 ((< 3 3) 'less)
			 (else 'equal)))
(applytester 2 'cond (cond ((assv 'b '((a 1) (b 2))) => cadr)
		     (else #f)))
(applytester 'composite 'case (case (* 2 3)
			 ((2 3 5 7) 'prime)
			 ((1 4 6 8 9) 'composite)))
(applytester 'consonant 'case (case (car '(c d))
			 ((a e i o u) 'vowel)
			 ((w y) 'semivowel)
			 (else 'consonant)))
(applytester #t 'and (and (= 2 2) (> 2 1)))
(applytester #f 'and (and (= 2 2) (< 2 1)))
(applytester '(f g) 'and (and 1 2 'c '(f g)))
(applytester #t 'and (and))
(applytester #t 'or (or (= 2 2) (> 2 1)))
(applytester #t 'or (or (= 2 2) (< 2 1)))
(applytester #f 'or (or #f #f #f))
(applytester #f 'or (or))
(applytester '(b c) 'or (or (memq 'b '(a b c)) (+ 3 0)))
(SECTION 4 2 2)
(applytester 6 'let (let ((x 2) (y 3)) (* x y)))
(applytester 35 'let (let ((x 2) (y 3)) (let ((x 7) (z (+ x y))) (* z x))))
(applytester 70 'let* (let ((x 2) (y 3)) (let* ((x 7) (z (+ x y))) (* z x))))
(applytester 70 'let* (let ((x 2) (y 3)) (let* ((x (+ x 5)) (z (+ x y))) (* z x))))
...
(SECTION 4 2 3)
(define x 0)
(applytester 6 'begin (begin (set! x 5) (+ x 1)))
(SECTION 4 2 4)
; (applytester '#(0 1 2 3 4) 'do (do ((vec (make-vector 5))
; 			    (i 0 (+ i 1)))
; 			   ((= i 5) vec)
; 			 (vector-set! vec i i)))
(applytester 25 'do (let ((x '(1 3 5 7 9)))
	       (do ((x x (cdr x))
		    (sum 0 (+ sum (car x))))
		   ((null? x) sum))))
...
; 		     neg)))))
(SECTION 4 2 6)
(applytester '(list 3 4) 'quasiquote `(list ,(+ 1 2) 4))
(applytester '(list a (quote a)) 'quasiquote (let ((name 'a)) `(list ,name ',name)))
(applytester '(a 3 4 5 6 b) 'quasiquote `(a ,(+ 1 2) ,@(map abs '(4 -5 6)) b))
(applytester '((foo 7) . cons)
	'quasiquote
...
(applytester '#(10 5 2 4 3 8) 'quasiquote `#(10 5 ,(sqt 4) ,@(map sqt '(16 9)) 8))
(applytester 5 'quasiquote `,(+ 2 3))
(applytester '(a `(b ,(+ 1 2) ,(foo 4 d) e) f)
	   'quasiquote `(a `(b ,(+ 1 2) ,(foo ,(+ 1 3) d) e) f))
(applytester '(a `(b ,x ,'y d) e) 'quasiquote
	   (let ((name1 'x) (name2 'y)) `(a `(b ,,name1 ,',name2 d) e)))
(applytester '(list 3 4) 'quasiquote (quasiquote (list (unquote (+ 1 2)) 4)))
...
;; Break here.
(let ((p (lambda (x) x)))
  (applytester #t eqv? p p))
(define gen-counter
 (lambda ()
   (let ((n 0))
      (lambda () (set! n (+ n 1)) n))))
(let ((g (gen-counter))) (applytester #t eqv? g g))
...
(let ((x '(a))) (applytester #t eq? x x))
(let ((x '#())) (applytester #t eq? x x))
(let ((x (lambda (x) x))) (applytester #t eq? x x))
...
(SECTION 6 3)
(applytester '(a b c d e) 'dot '(a . (b . (c . (d . (e . ()))))))
(define x (list 'a 'b 'c))
(define y x)
...
(applytester '(a b c . d) 'dot '(a . (b . (c . d))))
...
(applytester #t 'standard-case (eq? 'a 'A))
...
(applytester #f 'string->symbol (eq? 'bitBlt (string->symbol "bitBlt")))
...
(define (test-inexact)
  (let* ((f3.9 (string->number "3.9"))
    ...
    (newline)
    (SECTION 6 5 5)
    (applytester #t inexact? f3.9)
    (applytester #t 'inexact? (inexact? (max f3.9 4)))
    (applytester f4.0 'max (max f3.9 4))
    (applytester f4.0 'exact->inexact (exact->inexact 4))
    (applytester (- f5.0) round (- f4.5))
    (applytester (- f4.0) round (- f3.5))
    (applytester (- f4.0) round (- f3.9))
    (applytester f0.0 round f0.0)
    (applytester f0.0 round f.25)
    (applytester f1.0 round f0.8)
    (applytester f4.0 round f3.5)
    (applytester f5.0 round f4.5)
    (applytester 0.0000000002 * 2 0.0000000001)
    (applytester 2.0e-10 * 2 1.0e-10)
    (applytester 0.0000000002 * 2 1.0e-10)
    (applytester tinyfloat * 2 1.0e-10)
    (applytester reallytinyfloat * 2 1.0e-20)
    (applytester bigfloat * 2 1.0e+9 10.0)
    (applytester reallybigfloat * 2 1.0e+19 10)
    (comment
    ...
    (let ((x (string->number "4195835.0"))
      ...
      (applytester #t 'pentium-fdiv-bug (> f1.0 (- x (* (/ x y) y)))))))
...
(define (test-bignums)
  ...
  (applytester 24691357975308642
	     + 12345678987654321 12345678987654321)
  (applytester 0 - 12345678987654321 12345678987654321)
  (applytester -24691357975308642
	     + -12345678987654321 -12345678987654321)
  (applytester 0 - -12345678987654321 -12345678987654321)
  (applytester 0 + 12345678987654321 -12345678987654321)
  ...
  (applytester 0 modulo 3333333333 3)
  (applytester 0 modulo 3333333333 -3)
  (applytester 0 remainder 3333333333 3)
  (applytester 0 remainder 3333333333 -3)
  (applytester 2 modulo 3333333332 3)
  (applytester -1 modulo 3333333332 -3)
  (applytester 2 remainder 3333333332 3)
  (applytester 2 remainder 3333333332 -3)
  (applytester 1 modulo -3333333332 3)
  (applytester -2 modulo -3333333332 -3)
  (applytester -2 remainder -3333333332 3)
  ...
  (applytester 3 modulo 3 3333333333)
  (applytester 3333333330 modulo -3 3333333333)
  (applytester 3 remainder 3 3333333333)
  (applytester -3 remainder -3 3333333333)
  (applytester -3333333330 modulo 3 -3333333333)
  (applytester -3 modulo -3 -3333333333)
  (applytester 3 remainder 3 -3333333333)
  ...
  (applytester 0 modulo -2177452800 86400)
  (applytester 0 modulo 2177452800 -86400)
  (applytester 0 modulo 2177452800 86400)
  (applytester 0 modulo -2177452800 -86400)
  (applytester #t bigtest 281474976710655 65535)
  (applytester #t bigtest 281474976710654 65535)
  (SECTION 6 5 6)
  (applytester 281474976710655 string->number "281474976710655")
  (applytester "281474976710655" number->string 281474976710655)
  (applytester -281474976710655 string->number "-281474976710655")
  (applytester "-281474976710655" number->string -281474976710655)
  (applytester 281474976710655 string->number "FFFFFFFFFFFF" 16)
  (applytester 4503599627370495 string->number "FFFFFFFFFFFFF" 16)
  (applytester -100000000000000010 'big-minus (- (- 100000000000000000) 10)))
...
(define (applytest-cont)
  ...
  (newline)
  (SECTION 6 9)
  (applytester #t leaf-eq? '(a (b (c))) '((a) b c))
  (applytester #f leaf-eq? '(a (b (c))) '((a) b c d))
  (report-errs))
...
(define (applytest-delay)
  ...
  (newline)
  (SECTION 6 9)
  (applytester 3 'delay (force (delay (+ 1 2))))
  (applytester '(3 3) 'delay (let ((p (delay (+ 1 2))))
			(list (force p) (force p))))
  (applytester 2 'delay (letrec ((a-stream
