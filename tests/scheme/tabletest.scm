...
(define (edit-tests filename editfile)
  (if (file-exists? editfile)
	...
	(if (eq? (car in) 'dropped)
	    (if (test table '%items (cdr in))
		(begin (message "Drop didn't work")
		       (exit 1))
		(begin (message "Drop worked")
		       (store! table '%items (cdr in))
		       (dtype->file (cons 'stored (cdr in)) editfile)
		       (commit)))
	    (if (eq? (car in) 'stored)
		(if (equal? (get table '%items) (cdr in))
		    (begin (message "Store worked")
			   (remove-file editfile))
		    (begin (message "Store failed")
			   (remove-file editfile)
			   (exit 1)))
		(begin (message "Test inconsistency")
		       (exit 1)))))
      (let* ((table (table-from filename))
	     (item (pick-one (get table '%items))))
	(message "Dropping item " item)
	(drop! table '%items item)
	(dtype->file (cons 'dropped item) editfile)
	(commit))))
