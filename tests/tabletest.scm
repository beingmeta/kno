...
(use-module '{bench/randobj binio texttools})
...
(define (main filename (size #f) 
  ...
  (when (and (config 'reset) (string? filename))
    (when (file-exists? filename)
      (if (file-directory? filename)
	  (system "rm -rf " filename)	  
	  (remove-file filename)))
    (let ((edit-file (textsubst filename #("." (isalnum+) (eos)) ".edit.dtype")))
      (when (file-exists? edit-file) (remove-file edit-file))))
  (message "■■ TABLETEST " (write filename) 
    ...
    (printout " rthreads=" rthreads)
    (printout " wthreads=" wthreads))
