...
(use-module '{fileio binio logger texttools varconfig})
...
(defambda (init-oid-value! oid value)
  (if (config 'FORCEFRAMES)
      (store! oid 'values value)
      (set-oid-value! oid value)))
(defambda (read-oid-value oid)
  (if (config 'FORCEFRAMES)
      (get oid 'values)
      (oid-value oid)))
...
(define (makedb pool index files)
  ...
  (let* ((symbols-oid (allocate-oids pool))
    ...
    ;; The symbols OID
    (init-oid-value! symbols-oid allsyms)
    ...
    (let ((nums {}))
      ...
      (init-oid-value! numbers-oid nums))

    ;; The files OID
    (init-oid-value! files-oid (elts files))
    ...
    (add! noslots-oid 'x "x")
    ;; Reset the value
    (init-oid-value! noslots-oid (frame-create #f))
...
(define (checkoids pool index)
  ...
  (let* ((symbols-oid (find-frames index '%id 'symbols))
    ...
    (applytest #t symbol? (read-oid-value symbols-oid))
    (applytest #t number? (read-oid-value numbers-oid))
    (when (exists? (oid-value files-oid))
      (applytest #t string? (read-oid-value files-oid)))
    (applytest (file->dtypes (stringout dbsource "-symbols.dtype"))
	       read-oid-value symbols-oid)
    (applytest (file->dtypes (stringout dbsource "-numbers.dtype"))
	       read-oid-value numbers-oid)
    (applytest (file->dtypes (stringout dbsource "-files.dtype"))
	       read-oid-value files-oid)
...
(define (setup (source (or dbsource "testdb")) (opts #f))
  ...
  (makedb testpool testindex
	  '("r4rs.scm" "misctest.scm" "sequences.scm" "choices.scm")))
