...
(applytester #t some? odd? #(1 2 3 4 5 6))
(applytester #f some? odd? #(2 4 6))
(applytester #t every? even? #(2 4 6))
(applytester #(2 4 6) remove-if odd? #(1 2 3 4 5 6))
(applytester '(2 4 6) remove-if odd? '(1 2 3 4 5 6))
(applytester "CC" remove-if lowercase? "CamelCase")
...
(applytester 3 position 32 #"abc def")
(applytester 3 rposition 32 #"abc def")
(applytester 17 position #\u00e9  "a nice little café on the left bank")
(applytester 17 rposition #\u00e9  "a nice little café on the left bank")
(applytester 17 search "é" "a nice little café on the left bank")
(applytester 17 rposition #\u00e9  "a nice little café on the left bank")

(applytester #("foo" "bar" "quuz") remove "baz" #("foo" "bar" "baz" "quuz"))
(applytester "abcdegh" remove #\f "abcdefgh")
(applytester '("foo" "bar" "quuz") remove "baz" '("foo" "bar" "baz" "quuz"))
(applytester #"abcdegh" remove (char->integer #\f) #"abcdefgh")
...
(define-tester (seqtest seq len . pos.vals)
  (applytest len length seq)
...
(begin
  (applytester 1 position-if even-string? vec)
  (applytester 0 position-if odd-string? vec)
  (applytester 0 position-if-not even-string? vec)
  (applytester 1 position-if-not odd-string? vec)
  (applytester #f position-if even-string? vec2)
  (applytester #f position-if odd-string? vec2)

  (applytester 3 position-if even-string? vec 2)
  (applytester 2 position-if odd-string? vec 2)
  (applytester 2 position-if-not even-string? vec 2)
  (applytester 3 position-if-not odd-string? vec 2)
  (applytester #f position-if even-string? vec2)
  (applytester #f position-if odd-string? vec2)

  (applytester #f position-if even-string? vec 2 2)
  (applytester #f position-if even-string? vec 2 3)
  (applytester 2 position-if odd-string? vec 2 3)
  (applytester #f position-if even-string? vec2 1 3)
  (applytester #f position-if odd-string? vec2 1 3)

  (applytester 5 position-if even-string? vec -1 0)
  (applytester 6 position-if odd-string? vec -1 0)
  (applytester 6 position-if-not even-string? vec -1 0)
  (applytester 5 position-if-not odd-string? vec -1 0)
  (applytester #f position-if even-string? vec2 -1 0)
  (applytester #f position-if odd-string? vec2 -1 0)

  (applytester "2" find-if even-string? vec)
  (applytester "1" find-if odd-string? vec)
  (applytester "1" find-if-not even-string? vec)
  (applytester "2" find-if-not odd-string? vec)
  (applytester #f find-if even-string? vec2)
  (applytester #f find-if odd-string? vec2)
  )

(begin
  (applytester 1 position-if even-string? lst)
  (applytester 0 position-if odd-string? lst)
  (applytester 0 position-if-not even-string? lst)
  (applytester 1 position-if-not odd-string? lst)
  (applytester #f position-if even-string? lst2)
  (applytester #f position-if odd-string? lst2)

  (applytester 3 position-if even-string? lst 2)
  (applytester 2 position-if odd-string? lst 2)
  (applytester 2 position-if-not even-string? lst 2)
  (applytester 3 position-if-not odd-string? lst 2)
  (applytester #f position-if even-string? lst2)
  (applytester #f position-if odd-string? lst2)

  (applytester #f position-if even-string? lst 2 2)
  (applytester #f position-if even-string? lst 2 3)
  (applytester 2 position-if odd-string? lst 2 3)
  (applytester #f position-if even-string? lst2 1 3)
  (applytester #f position-if odd-string? lst2 1 3)

  (applytester 5 position-if even-string? lst -1 0)
  (applytester 6 position-if odd-string? lst -1 0)
  (applytester 6 position-if-not even-string? lst -1 0)
  (applytester 5 position-if-not odd-string? lst -1 0)
  (applytester #f position-if even-string? lst2 -1 0)
  (applytester #f position-if odd-string? lst2 -1 0)

  (applytester "2" find-if even-string? lst)
  (applytester "1" find-if odd-string? lst)
  (applytester "1" find-if-not even-string? lst)
  (applytester "2" find-if-not odd-string? lst)
  (applytester #f find-if even-string? lst2)
  (applytester #f find-if odd-string? lst2)
  )

(errtest (find-if))
(errtest (find-if even?))

(applytester "4" find-if even-string? vec 2)
(applytester "3" find-if odd-string? vec 2)
(applytester "3" find-if-not even-string? vec 2)
(applytester "4" find-if-not odd-string? vec 2)
(applytester #f find-if even-string? vec2)
(applytester #f find-if odd-string? vec2)

(applytester #f find-if even-string? vec 2 2)

(applytester #f find-if even-string? vec 2 3)
(applytester "3" find-if odd-string? vec 2 3)
(applytester #f find-if even-string? vec2 1 3)
(applytester #f find-if odd-string? vec2 1 3)

(applytester "6" find-if even-string? vec -1 0)
(applytester "7" find-if odd-string? vec -1 0)
(applytester "7" find-if-not even-string? vec -1 0)
(applytester "6" find-if-not odd-string? vec -1 0)
(applytester #f find-if even-string? vec2 -1 0)
(applytester #f find-if odd-string? vec2 -1 0)

;;; Pair tests

(define-tester (xseqtest seq (numeric #f))
  (when numeric
...
(applytester  {"three" "four" "five" "six" "seven" "eight" "nine" 
	     "ten" "eleven" "twelve" "thirteen"}
	    vector->elts 
	    vec)
(xseqtest vec)
...
(applytester #t veclen>=? vec 3)
(applytester #f veclen<=? vec 3)
(applytester #t veclen=? vec (length vec))
(applytester #f veclen>? vec 20)
(applytester #f veclen>=? vec 20)

(applytester #t seqlen>=? vec 3)
(applytester #f seqlen<=? vec 3)
(applytester #t seqlen=? vec (length vec))
(applytester #f seqlen>? vec 20)
(applytester #f seqlen>=? vec 20)
...
(let ((lst '("foo" "bar" "baz")))
  (applytester "foo" car lst)
  (applytester "bar" cadr lst)
  (applytester "baz" caddr lst)
  (set-car! (cddr lst) "quux")
  (applytester "quux" caddr lst)
  (applytester 3 length lst)
  (set-cdr! (cdr lst) '())
  (applytester 2 length lst))
