...
(define (get-pool (poolfile poolfile) (opts #f))
  (cond (testpool testpool)
	((equal? poolfile "zeropool") 
	 (set! memory-only #t)
	 (set! testpool (getpool @0/0))
	 (set! adjpool  (make-mempool "testpool" @0/0 (* 1024 1024)))
	 testpool)
	((equal? poolfile "mempool") 
	 (set! memory-only #t)
	 (set! testpool (make-mempool "testpool" @99/0 (* 1024 1024)))
	 (set! adjpoo (make-mempool "testpool" @99/0 (* 1024 1024)))
	 testpool)
...
(define (main (testcount 250) (poolfile poolfile) 
  ...
  (when (and reset (not memory-only))
    (when (file-exists? poolfile) 
      (if (file-directory? poolfile)
	  (system "rm -rf " poolfile)
	  (remove-file poolfile)))
    (when (file-exists? (adjfile poolfile))
      (if (file-directory? (adjfile poolfile))
	  (system "rm -rf " (adjfile poolfile))
